...
### 🧠 Как включить настоящий ИИ (бесплатно)

В веб-версии нажмите **⚙️** в шапке и укажите бесплатного провайдера:

| Провайдер | Как получить ключ | Модель |
|---|---|---|
| **Google Gemini** | https://aistudio.google.com/apikey | `gemini-2.0-flash` (60 запр/мин) |
| **OpenRouter** | https://openrouter.ai/keys (нужна регистрация) | `meta-llama/llama-3.3-70b-instruct:free` и др. |

Без ключа агент отвечает по встроенным шаблонам (база знаний СТК). С ключом — на любые вопросы,
при этом база знаний компании передаётся модели как контекст, поэтому она не выдумывает факты о СТК.
